/* The Market fee. Swaps are wallet-signed with the fee attached as a plain
   ...
   10 bps, from 15. Note what this does and does not do: the engine applies
   this fee to every venue including Jupiter, so it cancels out of any
   venue-versus-venue comparison inside Henar. What it changes is Henar's
   standing against the alternative of using an aggregator's own app directly,
   where the user pays Henar nothing; the cut is worth 5 bps to every routed
   user against that alternative. One constant serves the router engine and
   /api/market alike, so native and external routes are charged identically
   and neither can be charged twice. */

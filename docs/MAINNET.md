...
Status: the protected website preview is deployed; the Solana program is built but not deployed or funded. Helius and Jupiter credentials are configured. Pyth signup is pending. No funded mainnet flow or independent security audit has passed. The initial program configuration is paused.

Protected testing preview: [Stockroom](https://stockroom-ldk0kbkw9-arams-projects-cb250d30.vercel.app). Sign in with the existing Vercel account. This is a website deployment, not a deployed Solana program.
...
| `JUPITER_API_KEY` | Vercel server and worker | Market quotes and optional atomic solver routing |
| `PYTH_API_KEY` | Worker and local verification | Authenticated Hermes signed price updates |
...
Get Jupiter access through its [developer portal](https://developers.jup.ag/portal). Pyth requires authentication following the [August 26 upgrade](https://docs.pyth.network/price-feeds/core/upgrade/preparing); obtain a key through [Pyth Terminal](https://pythdata.app/). The existing receiver address was upgraded in place, so this integration retains its pinned SDK address and the authenticated `hermes.pyth.network` endpoint. Its full signed-update execution test still requires that key.
...
3. Run `npm run mainnet:cost`. It queries actual rent for the current executable. The checked development build was 787,448 bytes: approximately **4.0019 SOL persistent program rent**, with a conservative **8.0031 SOL peak rent allowance** including the temporary deployment buffer, before transaction fees and account setup. Recalculate after the final address/build; these are RPC rent results, not fiat price estimates.
...
8. Run `npm run protocol:preflight`. It verifies mainnet genesis, deployed executable bytes against the local build, vault/share mint, canonical programs, treasury, sealed manifest, extensions, configured credentials and authenticated oracle coverage. It does **not** certify a funded trade or economic safety.
...
The default route mode is inventory-only. Set `SOLVER_ROUTE_STOCKS=true` for atomic Jupiter routing. Fund the operational wallet with SOL for oracle posting/settlement rent and USDC for fronting stock swaps; settlement reimburses only after stock delivery. Keep operational balances limited. The worker does not have user withdrawal authority.
...
- Packs: buy, gift, explicit opening, real ORAO fulfillment, full Pyth posting, stock delivery, receipt display and replay rejection. Test sealed refunds and pending/selected timeout refunds. Earned packs require actual net yield, not principal funding.
...
- Authenticated quotes succeeded for all 39 candidate pack stocks. Pyth posting, complete swap simulation, issuer transfer eligibility and funded settlement remain unverified. No funded mainnet flow has passed yet.
